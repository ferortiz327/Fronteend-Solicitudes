...
export class AgendarVisitaComponent implements OnInit {
  ...
  guardarTareas(): void {
    ...
    const resultado = this.datosform.controls.fechaVisita.value;

    this.fechaAgenda = resultado.year + "-" + (resultado.month <= 9 ? "0" + resultado.month : resultado.month) + "-" + (resultado.day <= 9 ? "0" + resultado.day : resultado.day);

    if (this.fechaAgenda == "undefined-undefined-undefined")
    {AlertUtilities.showAlert({title:"Validación Agendar Visita",icon: IconAlerts.warning, message:"Debe seleccionar una fecha de visita"})}
    //console.log(this.fechaAgenda);

    const listarMisTareas = (): void => {
      this.cargarMisTareas();
    }
    this.bonitaService.guardarAgendarVisita(this.idTarea, resultado).subscribe({
      next: () => {
        this.showDetail = false;
        AlertUtilities.showAlert({ title: "Agendar Visita", icon: IconAlerts.success, message: "Se ha completado la tarea satisfactoriamente", functionAfterConfirm: listarMisTareas })
      },
